...
(see @tbl-variables for full descriptions)
...
: Description of dataset variables {#tbl-variables}
...
The first step was to examine how universities are distributed across countries. The results in @fig-geography show a strong geographic concentration.
...
```{python}
#| label: tbl-stats
#| tbl-cap: "Descriptive statistics of indicators (2015)"
#| code-fold: true
#| warning: false
#| message: false
...
stats = df_2015[METRICS].describe().T

stats = stats[['mean', 'std', 'min', '25%', '50%', '75%', 'max']]
stats.columns = ['Mean', 'Std', 'Min', '25%', 'Median', '75%', 'Max']
stats.index = [i.capitalize() for i in stats.index]
stats.index.name = "Indicator"

Markdown(stats.to_markdown(floatfmt=".2f"))
```
...
Overall the indicators differ in spread and shape, as shown in @tbl-stats and illustrated in @fig-histograms and @fig-box-plots, with `citations` being the most concentrated and `income` the most variable.
...
As shown in @fig-top-10, the United States and the United Kingdom universities dominate the top-10 list and prove their dominance in the ranking.

![Top 10 universities by total score (2015)](figures/top_10.png){#fig-top-10 fig-pos='H' width=100%}

At the national level the United States is both the most populated in terms of universities and the most successful. The other countries are clustering in @fig-scatter to indicate that high representation may not necessarily result in proportionately high average scores.
...
According to @fig-total, the overall average score varies with the time (between 57.7 and 62.5) and does not follow a specific trend, which implies that the ranking structure is usually stable.

![Average total score over time (2011–2016)](figures/total_score_per_year.png){#fig-total fig-pos='H' width=100%}

The major countries are ranked differently across the years, but one and the same group of countries has always been ranked at the top. This means that it is partially stable with some internal variation, which is illustrated in @fig-bump-charts.
...
The calculation of the gradient components (empirical weights) defined in (@eq-gradient) for the 2015 dataset reveals an almost perfect match between the official Times Higher Education methodology and the actual distribution of scores for the top-200 universities. 

```{python}
#| label: tbl-gradient
#| tbl-cap: "Gradient Analysis: Empirical vs Official Weights"
#| code-fold: true
#| warning: false
#| message: false
...
Markdown(gradient_results.to_markdown(index=False, floatfmt=".6f"))
```

The data confirms that the THE ranking strictly followed its stated logic in 2015, as summarized in @tbl-gradient and visualized in @fig-gradient: `teaching`, `research` and `citations` act as three equally powerful "heavyweight" levers, each accounting for approximately 30% of the total score. `international` and `income` provide significantly smaller "pushes" toward the top, at 7.5% and 2.5% respectively.
...
To ensure that our "gradient" approach (based on linear regression) is mathematically sound, we examine the linearity verification presented in @fig-linearity.
